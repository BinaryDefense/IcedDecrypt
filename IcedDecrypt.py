...
class IcedDecrypt:
    ...
    def main(self):
        ...
        #    print(decDict)
            ...
            filename = f"out_{hashlib.md5(outdata).hexdigest()}_{int(time.time())}_{self.output}"

            outfile = open(f"Assembled_Payloads_Debug/{filename}.file","w+b")
    ...
    def parseC2Buffer(self,dataDict): #read values from 0xA9 bytes large data block
        ...
        unkData = data[-0x14:]
        outDict = {"BuildID":buildID,"uri":uri.decode(),"c2_1":c2_1[1:-1].decode(),"c2_2":c2_2[1:-1].decode(),"UnknownData":base64.b64encode(unkData).decode()}
        return outDict
...
"""
        ...
        tempOut = b""
        print(dataSegments)
        segDict = {"SegmentIndex":0,"SegmentSize":0,"RawSegmentOffset":0,"VirtualSegmentOffset":0} #Initialize dicts for unpacking values
        bigDict = {"Segments":[],"ImageBase":imagBase,"ImageSize":imagSize,"ImageBaseEntry":imagEntry,"ImportTableOffset":impTableOff,"ImportTableSize":impTableSize}
        ...
        for i in range(dataSegments):
            ...
            segDict = {"SegmentIndex":0,"SegmentSize":0,"RawSegmentOffset":0,"VirtualSegmentOffset":0} #zero dict
            totalSize = int(0x1000 * (round((totalSize + dataSize) / 0x1000)))


            tempOut = outBuffer[:virtualOffset] + buffer[dataOff:dataOff+dataSize] + outBuffer[virtualOffset+dataSize:]
            outBuffer = tempOut
    ...
    def parseGZIPLoader(self,data):
        ...
        key = self.bruteforceKey(data[0xA+len(filename):]) #Using extracted data, bruteforce the key using a select batch of keys
...
"""
        flag = data[0:1]
        datfile_size = int.from_bytes(data[1:5],byteorder="little")
        dllfile_size = int.from_bytes(data[5:9],byteorder="little")
        dirname = self.extractStrFromBuff(data[9:])
        print("[+] Directory Name:",dirname)

        count = self.incNulls(data[9+len(dirname):])
        datname = self.extractStrFromBuff(data[count+9+len(dirname):])

        count = count + 9 + len(dirname) + len(datname)
        datname = datname[1:]
        ...
        count2 = self.incNulls(data[count:])
        count += count2
        rundllstr = self.extractStrFromBuff(data[count:])

        datfile_data = data[0xA9:0xA9+datfile_size]
        dllfile_start = 0xA9+datfile_size
    ...
    def genDropFiles(self,dataDict): #Generate files to write
        try:
            os.mkdir("Payloads")
        except Exception as e:
            print(e)
        try:
            os.mkdir(f"Payloads/{dataDict['Directory_Name']}")
        except Exception as e:
            print(e)
        datFileOut = f"Payloads/{dataDict['Directory_Name']}/{dataDict['DatFile_Name']}"
        dllFileOut = f"Payloads/{dataDict['Directory_Name']}/{dataDict['DllFile_Name']}"
        rundllOut = f"Payloads/{dataDict['Directory_Name']}/Rundll32Execution.txt"
        datFileData = base64.b64decode(dataDict["DatFile"])
        dllFileData = base64.b64decode(dataDict["DllFile"])
        ...
        dllout.close()
        rundllstr = dataDict["RunDll32_Execution"].replace(b"%s",b"{}").decode()
